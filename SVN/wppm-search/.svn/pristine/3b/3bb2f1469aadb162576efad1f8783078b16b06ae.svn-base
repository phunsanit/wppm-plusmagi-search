...
define('WP_SEARCH_FILE', __FILE__);
define('WP_SEARCH_URL', plugin_dir_url(__FILE__));
define('WP_SEARCH_PATH', plugin_dir_path(__FILE__));
...
class WP_Search
{
	...
	public function __construct()
	{
		...
		add_shortcode('wp_search', [$this, 'render_shortcode']);
	...
	public function register_blocks()
	{
		wp_register_script(
			'wp-search-block-js',
			WP_SEARCH_URL . 'assets/js/block.js',
			['wp-blocks', 'wp-element'],
			WP_SEARCH_VERSION,
			true
		);

		register_block_type('wp-search/search', [
			'editor_script'   => 'wp-search-block-js',
	...
	public function render_admin_page()
	{
		...
		<div class="wrap">
			...
			<p><?php esc_html_e('Thank you for using WP Search! This plugin provides a frontend search experience similar to the WordPress admin search, with role-based access control.', 'wp-search'); ?></p>

			<div class="card">
				<h2><?php esc_html_e('About the Developer', 'wp-search'); ?></h2>
				<p>
					<?php esc_html_e('For support, updates, and more information, please visit our website:', 'wp-search'); ?>
					<br>
					<a href="https://wp-search.plusmagi.com" target="_blank" rel="noopener noreferrer">
						<strong><?php esc_html_e('Visit wp-search.plusmagi.com →', 'wp-search'); ?></strong>
					</a>
				</p>
			</div>
		</div>
	...
	public function enqueue_scripts()
	{
		wp_enqueue_script(
			'wp-search-js',
			WP_SEARCH_URL . 'assets/js/search.js',
			['jquery'],
			WP_SEARCH_VERSION,
			true
		);
		...
		wp_enqueue_style(
			'wp-search-css',
			WP_SEARCH_URL . 'assets/css/search.css',
			['dashicons'],
			WP_SEARCH_VERSION
		);

		wp_localize_script('wp-search-js', 'wpSearch', [
	...
	public function render_shortcode()
	{
		ob_start();
		?>
		<div id="wp-search-wrapper">
			<input type="text" id="wp-search-input" placeholder="<?php esc_attr_e('Search...', 'wp-search'); ?>" autocomplete="off">
	...
	{
		register_rest_route('wp-search/v1', '/search', [
			'methods'  => 'GET',
 ...
 */
function wp_search_load_plugin() {
	WP_Search::get_instance();
}
add_action('plugins_loaded', 'wp_search_load_plugin');
