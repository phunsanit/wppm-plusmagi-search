jQuery(document).ready(function ($) {
	...
	function buildItem(item, mode) {
		...
		if (mode !== 'term' && item.status && item.status !== 'publish') {
			$('<span>').addClass('wp-search-status-pill').text(item.status).appendTo($title);
		}
	...
	function renderTabs(buckets) {
		...
		// Tab headers
		var $tabs = $('<div>').addClass('wp-search-tabs');
		$('<div>').addClass('wp-tab').attr('data-tab', 'posts').text('Posts (' + buckets.posts.length + ')').appendTo($tabs);
		$('<div>').addClass('wp-tab').attr('data-tab', 'categories').text('Category (' + buckets.categories.length + ')').appendTo($tabs);
		$('<div>').addClass('wp-tab').attr('data-tab', 'tags').text('Tag (' + buckets.tags.length + ')').appendTo($tabs);
	...
	$results.on('mousedown', '.wp-tab', function (e) {
	...
	$input.on('input', function () {
		...
		timer = setTimeout(function () {
			$.ajax({
				url: wpSearch.root + 'wp-search/v1/search',
				method: 'GET',
				data: { term: term },
	...
	$(document).on('click', function (e) {
		if (
			!$(e.target).closest('#wp-search-input').length &&
			!$(e.target).closest('#wp-search-results').length
		) {
			$results.hide();
		}
	});
